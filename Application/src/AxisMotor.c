...
unsigned char AxisMotor_Control(unsigned char nFinalAxisMotorState,unsigned short stopLevel,unsigned int speed)
{
  ...
  unsigned short ncurPosition;// = Input_GetAxisMotorPosition();
  //static unsigned short oldPosition;
    static unsigned short oldPosition;
  unsigned short stopPosition;
  int levelMax = _3D_POSITION_POINT;
  curPosition = Input_GetAxisMotorPosition();
  ncurPosition = Input_GetAxisMotorPosition();
  ncurPositiont = Input_GetAxisMotorPosition(); 
   ...
   speed = _3D_SPEED_8;
    ...
    case 1:
      if(Knead_Max_Ok)
      {
        ...
        if(ncurPosition < 7) speed = _3D_SPEED_2;
   ...
   case STATE_RUN_AXIS_3D_KNEAD:
    ...
    case 1:
         ...
         else
         {
            ...
            AxisMotor_Break(); 
            _3D_step = 0;
            nRetVal = 1;     //20170630    wgh20180422
         }
         break;
    case 0:
       if(downWalkRun)
...
//       }
      ...
      bPowerFlag = TRUE;
      _3D_Min_Ok = 0;
wghncurPositiont = Input_GetAxisMotorPosition(); 
  ...
  case STATE_RUN_AXIS_REAL_POSITION: //3D马达实际位置
    ...
    if((ncurPosition <= stopPosition + 1) && (ncurPosition >= stopPosition - 1 ))
    { 
      bPowerFlag = FALSE;
      nRetVal = TRUE ;
      AxisMotor_Break();
  ...
  case STATE_RUN_AXIS_POSITION:
    ...
    if((ncurPosition <= stopPosition + 2) && (ncurPosition >= stopPosition - 2 ))
    {  //到达位置区域
      bPowerFlag = FALSE;
      nRetVal = TRUE ;
      AxisMotor_Break();
  ...
  case STATE_RUN_AXIS_F_BEHIND:  //按摩头向后 按摩力道减小，碰到后行程开关才停  
    ...
    if(ncurPosition < 5) speed = _3D_SPEED_2;
    bDisableCheckCurrent = TRUE;
    //AxisMotor_ClockRun();
  ...
  case STATE_RUN_AXIS_F_FORWARD:   //按摩头向前 按摩力道加大 碰到前行程开关才停
    ...
    if(ncurPosition > 35) speed = _3D_SPEED_5;
   // AxisMotor_UnClockRun();
    AxisMotor_ClockRun();
    bPowerFlag = TRUE;
    break ;
